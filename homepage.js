...
const storyData = [
    ...
    { file: 'stories/europeengland2.html', title: 'The Black Death', continent: 'europe', country: ['england', 'france', 'germany'], image: "https://images.aeonmedia.co/images/b055a78f-b44e-4547-82c9-fe1bb124e920/essay-prado-the-triumph-of-death.jpg", keywords: ['england', 'europe', 'plague', 'medieval'] },
...
function toggleSaveStory(storyFile) {
    let saved = JSON.parse(localStorage.getItem('savedStories') || '[]');
    
    if (saved.includes(storyFile)) {
        saved = saved.filter(file => file !== storyFile);
    } else {
        saved.push(storyFile);
    }

    localStorage.setItem('savedStories', JSON.stringify(saved));

    updateSaveButtonUI(storyFile);
}

function updateSaveButtonUI(storyFile) {
    let saved = JSON.parse(localStorage.getItem('savedStories') || '[]');
    ...
    if (saveIcon && saveText) {
        if (saved.includes(storyFile)) {
            ...
            saveText.textContent = 'Save Story';
        }
    }
}
